        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="apple-glass rounded-2xl max-w-md w-full p-6 relative animate-in fade-in">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="bg-emerald-600 text-white text-3xl font-black text-center py-3 rounded-xl mx-auto max-w-[200px]">
                ₹ {receiptModal.data?.amount || '0'}
              </div>
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={() => { const w = window.open(); if (w) { w.document.write('<pre>' + JSON.stringify(receiptModal.data, null, 2) + '</pre>'); w.print(); } }} className="flex-1 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-sm font-medium transition flex items-center justify-center gap-2">
                <Download className="w-4 h-4" /> Download
              </button>
              <button onClick={() => { const text = `Receipt: ${receiptModal.data?.receiptNo}\nAmount: ₹${receiptModal.data?.amount}\nDonor: ${receiptModal.data?.name}`; window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank'); }} className="flex-1 py-2 bg-emerald-600 hover:bg-emerald-500 rounded-lg text-sm font-medium transition flex items-center justify-center gap-2">
